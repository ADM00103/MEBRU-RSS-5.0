...
function curl_get_contents(string $url): string|false
{
    $ch = curl_init();
...
function clean_html(string $html): string
{
    ...
    $html = preg_replace('~<p>\s*Подпишись\s+vk\.ru/mebru\s*</p>~isu', '', $html);
    return trim($html);
}

function get_item_text(SimpleXMLElement $item): string
{
    $content = '';
...
function get_pub_date(SimpleXMLElement $item): string
{
    ...
    if (!empty($item->children('dc', true)->date)) {
        $ts = strtotime((string)$item->children('dc', true)->date);
        if ($ts !== false) {
            return date(DATE_RSS, $ts);
        }
    }
...
function fetch_rss(string $url, string $source_name, int $limit = 20): array
{
    ...
    foreach ($rss->channel->item as $item) {
        ...
        $description = get_item_text($item);
...
usort($all_items, function (array $a, array $b): int {
...
<?php foreach ($all_items as $item): ?>
    <item>
        ...
        <description><![CDATA[
<?= $item['description'] ?>
<p><strong>Подпишись vk.ru/mebru</strong></p>
        ]]></description>
    </item>
<?php endforeach; ?>
</channel>

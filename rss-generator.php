...
function extract_text($item) {
    ...
    if (isset($namespaces['content'])) {
        ...
        if (!empty($content->encoded)) {
            $text = (string)$content->encoded;
        }
    }

    if ($text === '' && !empty($item->description)) {
...
foreach ($sources as $source) {
    $source_items = fetch_rss($source['url'], $source['name'], $items_per_source);
    if (!empty($source_items)) {
        $all_items = array_merge($all_items, $source_items);
    }
}
...
file_put_contents($output_file, $xml_output);
echo $xml_output;